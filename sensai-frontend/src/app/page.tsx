            ...
            <>
              ...
              {showSegmentedTabs && (
                <div className="flex justify-center mb-10">
                  <div className="inline-flex rounded-2xl p-1.5 w-full sm:w-auto bg-[#e5e7eb]/80 dark:bg-[#222222]/80 backdrop-blur-sm border border-black/5 dark:border-white/5 shadow-inner">
                    {tabsToShow.includes('teaching') && (
                      <button
                        className={`flex items-center justify-center px-2 sm:px-6 py-2.5 rounded-xl text-sm font-medium cursor-pointer flex-1 sm:flex-initial transition-all duration-300 ${activeTab === 'teaching'
                          ? 'bg-white dark:bg-[#333333] text-black dark:text-white shadow-sm scale-[1.02]'
                          : 'text-[#4b5563] dark:text-[#9ca3af] hover:text-black dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5'
                          }`}
                    ...
                    {tabsToShow.includes('mentoring') && (
                      <button
                        className={`flex items-center justify-center px-2 sm:px-6 py-2.5 rounded-xl text-sm font-medium cursor-pointer flex-1 sm:flex-initial transition-all duration-300 ${activeTab === 'mentoring'
                          ? 'bg-white dark:bg-[#333333] text-black dark:text-white shadow-sm scale-[1.02]'
                          : 'text-[#4b5563] dark:text-[#9ca3af] hover:text-black dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5'
                          }`}
                    ...
                    {tabsToShow.includes('learning') && (
                      <button
                        className={`flex items-center justify-center px-2 sm:px-6 py-2.5 rounded-xl text-sm font-medium cursor-pointer flex-1 sm:flex-initial transition-all duration-300 ${activeTab === 'learning'
                          ? 'bg-white dark:bg-[#333333] text-black dark:text-white shadow-sm scale-[1.02]'
                          : 'text-[#4b5563] dark:text-[#9ca3af] hover:text-black dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5'
                          }`}
              ...
              <div className="mb-10">
                {!hasTeachingCourses && !hasMentoringCourses && !hasLearningCourses ? (
                  ...
                  <div className="text-center py-20 bg-gray-50/50 dark:bg-[#111111]/50 rounded-3xl border border-gray-100 dark:border-[#222222]">
                    <h2 className="text-3xl font-medium tracking-tight mb-3 text-foreground">What if your next big idea became a course?</h2>
                    <p className="text-muted-foreground text-lg mb-8">It might be easier than you think.</p>
                    <div className="flex justify-center gap-4">
                      <button
                        onClick={handleCreateCourseButtonClick}
                        className="px-8 py-3.5 text-sm font-medium rounded-full hover:scale-105 hover:shadow-lg transition-all duration-300 inline-block cursor-pointer bg-foreground text-background"
                      >
                        Create course
                      </button>
                    </div>
                  ...
                  <h2 className="text-3xl tracking-tight font-medium text-foreground px-2">
                    {getSingleRoleTitle()}
                  </h2>
                )}
              </div>
